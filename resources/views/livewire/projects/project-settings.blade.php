<div>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            <a href="{{ route('projects.board', $project) }}" class="text-yt-muted hover:text-yt-text">{{ $project->name }}</a>
            <span class="text-yt-faint">/</span>
            <h1 class="font-semibold">Настройки</h1>
        </div>
    </x-slot>
    ...
    <div class="mx-auto max-w-3xl space-y-8 p-6">
        ...
        <section class="rounded-lg border border-yt-border-soft bg-yt-panel">
            <div class="border-b border-yt-border-soft px-5 py-3">
                ...
                <p class="mt-0.5 text-xs text-yt-faint">Финальный статус означает «завершено»: задачу нельзя перевести в него, пока не закрыты её зависимости. Хотя бы один финальный статус должен существовать всегда.</p>
            </div>
                ...
                @foreach ($statuses as $index => $status)
                    <li class="flex items-center gap-3 px-5 py-2.5" wire:key="status-{{ $status->id }}">
                        ...
                        <label class="flex cursor-pointer items-center gap-1.5 text-xs text-yt-muted">
                            <input type="checkbox" @checked($status->is_final)
